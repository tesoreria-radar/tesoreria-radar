...
export async function GET(request: Request) {
  ...
  if (scoreValue != null && scoreValue < 60) {
    alerts.push({ level: 'CRITICA', title: 'Health Score degradado', detail: `Score operativo ${scoreValue}/100.`, why: 'La combinación de liquidez, mercado, riesgo y calidad de datos está por debajo del umbral operativo.' });
  }
  if (mepVsOfficial != null && mepVsOfficial > 5) {
    alerts.push({ level: 'ALTA', title: 'Brecha MEP elevada', detail: `MEP ${mepVsOfficial.toFixed(2)}% por encima del oficial.`, why: 'La brecha supera el umbral operativo del 5%.' });
  }
  for (const event of urgentEvents.slice(0, 3)) {
    alerts.push({ level: event.daysUntil <= 3 ? 'CRITICA' : 'ALTA', title: event.title, detail: `${event.type} · ${event.daysUntil} días.`, why: 'La obligación/evento está dentro de la ventana de atención inmediata.' });
  }
  ...
  const priority1 = urgentEvents.filter((x: any) => x.daysUntil <= 3).slice(0, 3).map((x: any) => ({
    title: x.title, type: x.type, dueDate: x.date, daysUntil: x.daysUntil, source: x.source,
    why: `Vence en ${x.daysUntil} día${x.daysUntil === 1 ? '' : 's'}; requiere cobertura de caja y seguimiento inmediato.`,
  }));

  const priority2 = [
    mepVsOfficial != null && mepVsOfficial > 5 ? {
      title: 'Revisar timing y exposición cambiaria', reason: `Brecha MEP ${mepVsOfficial.toFixed(2)}%`,
      why: 'La brecha MEP/oficial supera 5%; conviene revisar timing antes de ejecutar operaciones cambiarias.', source: fx?.sources?.mep ?? null,
    } : null,
    data.bna?.sourceStatus?.billete !== 'OK' ? {
      title: 'Validar cotización BNA billete', reason: 'Fuente BNA no confirmada',
      why: 'La fuente directa no confirmó billete de forma inequívoca; el sistema no lo reemplaza por otra cotización.', source: data.bna?.source ?? null,
    } : null,
    fx?.anomaly?.status === 'ANOMALIA' ? {
      title: 'Revisar anomalía MEP', reason: `z-score ${typeof fx.anomaly.zScore === 'number' ? fx.anomaly.zScore.toFixed(2) : 'N/D'}`,
      why: 'La observación MEP se encuentra a al menos 2 desviaciones estándar de su ventana reciente observada.', source: fx?.sources?.mep ?? null,
    } : null,
  ].filter(Boolean);

  const priority3 = events.filter((x: any) => x.daysUntil > 3 && x.daysUntil <= 14).slice(0, 5).map((x: any) => ({
    title: x.title, type: x.type, dueDate: x.date, daysUntil: x.daysUntil, source: x.source,
    why: `Agenda de corto plazo: quedan ${x.daysUntil} días para preparar caja y documentación.`,
  }));
  ...
  const sourceHealth = {
    dashboard: data.dashboard ? 'OK' : 'ERROR',
    fima: data.fima?.ok ? 'OK' : 'ERROR',
    score: data.score?.ok ? 'OK' : 'ERROR',
    calendar: data.calendar?.ok ? 'OK' : 'ERROR',
    fx: data.fx?.ok ? 'OK' : 'ERROR',
    bnaBillete: data.bna?.sourceStatus?.billete === 'OK' ? 'OK' : 'ERROR',
    bnaDivisa: data.bna?.sourceStatus?.divisa === 'OK' ? 'OK' : 'ERROR',
  };
  ...
  return NextResponse.json({
    ...
    dataHealth: { status: failures.length ? 'ATENCION' : 'OK', sources: sourceHealth, failures },
    analytics: { mepAnomaly: fx?.anomaly ?? null },
