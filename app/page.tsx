...
type Fima = { ok?: boolean; checkedAt?: string; source?: string; updatedAt?: string; fund?: string; currency?: string; category?: string; redemption?: string; reference?: { thirtyDay: number | null; ytd: number | null; twelveMonth: number | null; tnaEstimated: number | null }; classes?: Record<string, { thirtyDay: number; ytd: number; twelveMonth: number; tnaEstimated: number }> };
type DashboardData = {
  checkedAt?: string;
  sources?: { oficial?: string | null; mep?: string | null; inflation?: string | null };
  market?: { oficial?: { compra: number | null; venta: number | null; updatedAt?: string | null }; mep?: { compra: number | null; venta: number | null; updatedAt?: string | null }; brechaMepOficial?: number | null };
  inflation?: { month: string; value: number } | null;
};

const nav = ['Resumen', 'Alertas', 'Mercado', 'Money Market', 'ARCA', 'Cash Calendar', 'BCRA', 'Real Estate', 'Vaca Muerta'];
...
const modules = [
  ...
  ['Money Market', 'FIMA Premium · TNA · 30 días · 12 meses'],
...
function SectionContent({ section, data, status, fima }: { section: string; data: DashboardData; status: Status; fima: Fima }) {
  ...
  if (section === 'Money Market') return <>
    <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14}}>
      <Card title="FIMA Premium · 30 días" value={pct(fimaB?.thirtyDay)} detail="Clase B · rendimiento neto" />
      <Card title="TNA estimada" value={pct(fimaB?.tnaEstimated)} detail="Anualización de 30 días" />
      <Card title="12 meses" value={pct(fimaB?.twelveMonth)} detail="Rendimiento acumulado" />
      <Card title="Rescate" value="T+0" detail="Liquidez inmediata" />
    </div>
    <Panel title="FIMA Premium · indicador de tesorería">
      <Row label="Clase B · 30 días" value={pct(fimaB?.thirtyDay)} />
      <Row label="Clase B · YTD" value={pct(fimaB?.ytd)} />
      <Row label="Clase B · 12 meses" value={pct(fimaB?.twelveMonth)} />
      <Row label="TNA estimada" value={pct(fimaB?.tnaEstimated)} />
      <Row label="Último cierre disponible" value={fima.updatedAt || '—'} />
      <Row label="Categoría" value={fima.category || 'Money Market ARS'} />
      <Row label="Fuente" value="CAFCI · datos procesados en Fonditos" />
    </Panel>
    <Panel title="Comparación por clase">
      <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12}}>{(['A','B','C'] as const).map(k=>{const c=fima.classes?.[k]; return <div key={k} style={{border:'1px solid rgba(124,162,204,.15)',borderRadius:11,padding:15,background:'rgba(255,255,255,.018)'}}><div style={{fontWeight:800}}>Clase {k}</div><div style={{marginTop:8,color:'#8fa5be'}}>30d <b style={{color:'#e8eef8'}}>{pct(c?.thirtyDay)}</b></div><div style={{marginTop:5,color:'#8fa5be'}}>12m <b style={{color:'#e8eef8'}}>{pct(c?.twelveMonth)}</b></div><div style={{marginTop:5,color:'#8fa5be'}}>TNA <b style={{color:'#5ee7a0'}}>{pct(c?.tnaEstimated)}</b></div></div>})}</div>
      <div style={{fontSize:11,color:'#71869f',marginTop:14}}>Los rendimientos son netos de honorarios según el valor de cuotaparte publicado. Rendimientos pasados no garantizan resultados futuros.</div>
    </Panel>
  </>;
