...
type Command = {
  ...
  priorities?: { one?: { title: string; type: string; dueDate: string; daysUntil: number; source?: string }[]; two?: { title: string; reason: string; source?: string | null }[]; three?: { title: string; type: string; dueDate: string; daysUntil: number; source?: string }[] };
 ...
 return <main style={{minHeight:'100vh',background:'radial-gradient(circle at 80% 0%,#163761 0,#08111f 42%,#040810 100%)',color:'#e8eef8',fontFamily:'Inter,system-ui',padding:28}}>
  <div style={{maxWidth:1400,margin:'0 auto'}}>
   ...
   <section style={{marginTop:18,display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14}}>
    {[['PRIORIDAD 1',d.priorities?.one||[],'#ff6b6b'],['PRIORIDAD 2',d.priorities?.two||[],'#ffd166'],['PRIORIDAD 3',d.priorities?.three||[],'#77b7ff']].map(([title,items,tone])=><div key={title as string} style={{background:'rgba(13,23,38,.82)',border:`1px solid ${tone}44`,borderRadius:14,padding:20,minHeight:150}}><div style={{fontSize:12,color:tone as string,letterSpacing:1.5,fontWeight:800}}>{title as string}</div>{(items as any[]).length===0?<div style={{marginTop:18,color:'#71869f'}}>Sin tareas pendientes</div>:(items as any[]).map((x:any,i)=><div key={i} style={{marginTop:14}}><b>{x.title}</b><div style={{fontSize:11,color:'#8196b0',marginTop:4}}>{x.type||x.reason||''}{x.daysUntil!=null?` · ${x.daysUntil} días`:''}</div></div>)}</div>)}
   </section>
   ...
   <footer style={{marginTop:20,fontSize:11,color:'#647b96'}}>Motor agregado: mercado · FX · BNA · FIMA · Health Score · Cash Calendar. Prioridad 1 = ≤3 días; Prioridad 2 = señales de mercado/calidad; Prioridad 3 = agenda ≤14 días. Los datos faltantes permanecen explícitos.</footer>
  </div></main>
}
